...
. `Formatted_docs` - Contains the latest HTML file of the metadata convention. You can view this file in your web browser by going https://htmlpreview.github.io/?https://github.com/ices-gists/wg_WGFAST/blob/master/Formatted_docs/TG-AcMeta.html[here].
. `.github/workflows` - Contains the `.yml` for a GitHub Action to build the HTML and put it in `Formatted_docs`. Do not touch this file, please. +
{empty} +
