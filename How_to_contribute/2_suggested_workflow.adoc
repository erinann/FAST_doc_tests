...
PS - Contributions to any of the help documents are welcome. :smile: +
...
Once the repository is forked to your account you can either <<Editing documents in GitHub, edit the documents directly in GitHub>> or clone the repository to your computer and work locally. *We suggest cloning the repository to your local machine unless you are only going to fix a few typographical error.*
{empty} +
...
1. Open the Git command line and use the `cd` command to navigate to directory (folder) you want to copy the repository to.  +
...
TIP: In most systems you can type the first few characters of the folder and use `Tab` to autocomplete.
...
** terminal-tab +
 To use Git from within Atom. This is a little flakey. Closing the terminal within the editor crashes Atom.
 ...
 * Brackets Git +
 Integration of Git into Bracket. This is a little flakey. If you have two-factor authentication setup in GitHub, you cannot push commits to GitHub because the authentication fails. +
 {empty} +
...
=== Stage and commit
After you have made a series of edits, save the document, stage it (`add` command), and commit it to your local repository with a useful commit message. Staging tells Git which files in your working directory have changed and should be commited to the repository. You can continue to edit files that have been stages. It is best practice to commit changes often to keep a record of your work. For example, if you finish working on an introduction section, save it and commit the document. +
...
When you create a fork of a repository you only have the version of the files that are in the repository at that time. Assume you made corrections and edits to the metadata convention document and submitted a pull request that was accepted (Woo hoo! Good job!). A couple days later several other contributors submit pull requests that were accepted. Now you have an old copy of the forked repository and are out of sync with the upstream repo. To update your forked repository you have to "synchronize your fork", also known as "getting upstream updates".
